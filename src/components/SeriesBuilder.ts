...
export class SeriesBuilder {
  public static buildCandlestickSeries(
    marketData: OHLCV[],
  ...
  public static buildIndicatorSeries(
    ...
    indicators.forEach((indicator, id) => {
      ...
      if (indicator.paneIndex > 0) {
        ...
        const confIndex = paneLayout.findIndex(
          (p) => p.index === indicator.paneIndex
        );
        if (confIndex !== -1) {
      ...
      Object.keys(indicator.plots).forEach((plotName) => {
        ...
        plot.data.forEach((point) => {
          ...
          if (index !== undefined) {
            ...
            colorArray[index] = point.options?.color || plot.options.color;
          }
        });
